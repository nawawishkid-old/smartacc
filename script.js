window.addEventListener("load", function() {
  ...
  var allNavTab = document.querySelectorAll(".nav-tab"),
      ovWrapper = document.getElementById("overviewWrapper"),
      recWrapper = document.getElementById("addRecordWrapper");
  ...
  for(var i = 0; allNavTab[i]; i++) {
    allNavTab[i].addEventListener("click", function() {
      if(this.id == "overviewNavTab") {
        ovWrapper.classList.remove("hide");
        recWrapper.classList.add("hide");
      } else {
        recWrapper.classList.remove("hide");
        ovWrapper.classList.add("hide");
      }
    })
  }

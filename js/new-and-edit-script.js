window.addEventListener("load", function() {
  ...
  // --- 2.1 IF DOM PAGE IS 'TODAY-EDIT.PHP', SELECT INITIAL OPTION ON [SELECT] ELEMENT ---
  if(window.location.href === "http://localhost/smartacc/today-edit.php") {
    console.log("This page is today-edit.php");
  ...
  // --- 2. FUNCTION ---
  // Fetch subinput and store its value when the value is changed.
  ...
  // Store recently-selected value into data-recent-[value,in-value,ex-value].
  ...
  submitBtn.addEventListener("click", function() {
    ...
    // Create AJAX
    var xhr = new XMLHttpRequest();
    xhr.open("POST", "php/new-and-edit-php.php");
    xhr.send(fd);
    xhr.onload = function() {
      console.log("--- xhr.onload ---");
